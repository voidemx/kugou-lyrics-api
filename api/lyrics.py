...
class Lyrics:
    ...
    TIME_TAG_REGEX = re.compile(r"\[\d{1,2}:\d{1,2}\.\d{2,3}\]")
    ...
    def get_lyrics_by_query(
        ...
        if candidate:
            ...
            synced_content = self.normalize_content(dl.get("content", "")) if dl else ""
            
            if synced_content:
                plain_content = self.TIME_TAG_REGEX.sub("", synced_content).strip()
                return {"syncedLyrics": synced_content, "plainLyrics": plain_content}
